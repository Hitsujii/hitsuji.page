...
2. Do not memorize everything. Return to lessons and documentation.
...
1. C++ started as an extension of C, but it is not just C with add-ons.
...
2. They can make code work on your machine even though it is not standard C++.
...
3. `/* */` creates a block comment, but do not nest block comments.
...
10. Do not define multiple variables on one line.
...
8. Data that `std::cin` does not extract remains in the buffer.
9. When reading into a variable, still initialize it, e.g. `int x {};`.

## 1.6

1. C++ usually does not automatically initialize local variables of simple types, e.g. `int x;`.
2. An `uninitialized variable` is a variable that has not yet received a known value.
...
7. `undefined behavior` means the C++ standard does not define what should happen.
...
1. `keywords` are reserved C++ words, e.g. `int`, `return`, `if`; you cannot use them as your own names.
2. An `identifier` is a name given to something in code, e.g. a variable, function, or type.
3. An identifier can contain letters, digits, and `_`, but it cannot start with a digit.
...
8. Do not start your own names with `_`, because such names may be reserved.
...
5. C++ is whitespace-independent, so formatting usually does not change meaning, but it affects readability.
...
2. A literal has a value and a type, but its value is fixed and cannot be changed.
...
4. An expression does not end with a semicolon. The semicolon ends the statement containing the expression.
...
3. If input will be used later, do not overwrite it unnecessarily.
...
8. C++ does not allow defining functions inside other functions.
...
8. `main()` must return `int` and should not be called manually.
...
1. `void` as a return type means the function does not return a value.
...
4. Do not put `return;` at the end of a `void` function, because it is unnecessary.
5. `return;` without a value can make sense in a `void` function if you want to exit early.
6. A `void` function cannot be used where an expression producing a value is required.
...
9. An `unreferenced parameter` is a parameter that exists but is not used in the function body.
...
6. A local variable from one function is not visible in another function.
...
9. Changing a parameter passed by value does not change the variable in the caller.
...
5. Do not combine calculating a result and printing a result in one function if they can be separated cleanly.
...
12. ODR means you cannot define the same thing twice in the same scope.
...
3. The compiler compiles each file separately and does not remember the contents of other files.
...
9. Do not include `.cpp` files. Compile them together.

## 2.9

1. A `naming collision` is a name conflict where two identifiers have the same name and the compiler or linker cannot distinguish them.
...
2. The `preprocessor` makes textual changes to code, but it does not modify the original files.
...
9. Do not include `.cpp` files. Add them to compilation.
...
6. A header guard does not block including the same header in different `.cpp` files.
7. A header guard does not replace the rule: declarations in `.h`, function definitions in `.cpp`.
8. `#pragma once` does something similar and is convenient, but it is not part of the C++ standard.
...
7. Do not write the whole program at once.
8. First make a simple working version, then expand it.
9. Do not polish the first code version too early.
